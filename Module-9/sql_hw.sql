...
-- 1b
SELECT CONCAT_WS(" ",first_name,last_name) "Actor Name" 
FROM actor;
...
(
 SELECT p.staff_id 
 FROM payment 
 WHERE LOCATE("2005-08",p.payment_date)
) 
 GROUP BY staff_id; 
...
SELECT i.film_id, f.title, COUNT(i.inventory_id) AS "Inventory" 
FROM inventory AS i 
INNER JOIN film AS f ON i.film_id = f.film_id 
WHERE f.title = 'Hunchback Impossible'; -- GROUP BY film_id;
